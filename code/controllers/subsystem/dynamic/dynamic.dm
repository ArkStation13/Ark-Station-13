...
/datum/controller/subsystem/dynamic/proc/generate_threat()
	...
	var/calculated_max_threat = (GLOB.clients.len < low_pop_player_threshold) ? LERP(low_pop_maximum_threat, max_threat_level, GLOB.clients.len / low_pop_player_threshold) : max_threat_level // ARK STATION EDIT || var/calculated_max_threat = (GLOB.clients.lenReady < low_pop_player_threshold) ? LERP(low_pop_maximum_threat, max_threat_level, GLOB.clients.lenReady / low_pop_player_threshold) : max_threat_level
	log_dynamic("Calculated maximum threat level based on player count of [GLOB.clients.len]: [calculated_max_threat]") // ARK STATION EDIT || log_dynamic("Calculated maximum threat level based on player count of [GLOB.clients.lenReady]: [calculated_max_threat]")
...
/datum/controller/subsystem/dynamic/proc/setup_parameters()
	...
	SSblackbox.record_feedback(
		...
		list(
			...
			"max_threat" = (GLOB.clients.len < low_pop_player_threshold) ? LERP(low_pop_maximum_threat, max_threat_level, GLOB.clients.len / low_pop_player_threshold) : max_threat_level, // ARK STATION EDIT || "max_threat" = (GLOB.clients.lenReady < low_pop_player_threshold) ? LERP(low_pop_maximum_threat, max_threat_level, GLOB.clients.lenReady / low_pop_player_threshold) : max_threat_level,
			"player_count" = GLOB.clients.len, // ARK STATION EDIT || "player_count" = GLOB.clients.lenReady,
