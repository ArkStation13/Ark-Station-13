...
/datum/outfit
	...
	// ARK STATION EDIT - Extra Inventory - use the correct paths
...
/datum/outfit/proc/equip(mob/living/carbon/human/user, visualsOnly = FALSE)
	...
	if(ears)
		EQUIP_OUTFIT_ITEM(ears, ITEM_SLOT_EARS_LEFT) // ARK STATION EDIT - Extra inventory
	if(glasses)
	...
	if(undershirt)
		w_shirt = w_shirt || initial(undershirt.shirt_obj) //ARK STATION EDIT - Extra inventory - original: user.undershirt = initial(undershirt.name)

	if(bra)
		w_bra = w_bra || initial(bra.bra_obj) //ARK STATION EDIT - Extra inventory - original: user.bra = initial(bra.name)

	if(underwear)
		w_underwear = w_underwear || initial(underwear.briefs_obj) //ARK STATION EDIT - Extra inventory - original: user.underwear = initial(underwear.name)

	if(socks)
		w_socks = w_socks || initial(socks.socks_obj) //ARK STATION EDIT - Extra inventory - original: user.socks = initial(socks.name)
...
/datum/outfit/proc/get_chameleon_disguise_info()
	...
	w_underwear, w_socks, w_shirt, w_bra, ears_extra, wrists) // ARK STATION EDIT - Extra inventory	types += chameleon_extras
	types += skillchips
