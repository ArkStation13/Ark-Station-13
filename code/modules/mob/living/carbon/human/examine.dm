...
//this is very slightly better than it was because you can use it more places. still can't do \his[src] though.
	...
	var/h = 0
	if(!dna.species.lore_protected && dna.species.id == SPECIES_TESHARI)
		h = round((dna.current_body_size + ((mob_height - HUMAN_HEIGHT_MEDIUM)/100)) * 110,5)
	else
		h = round((dna.current_body_size + ((mob_height - HUMAN_HEIGHT_MEDIUM)/100)) * 180,5)
	...
	*/ //NOVA EDIT END
	...
	var/obscurity_examine_pref = (client?.prefs?.read_preference(/datum/preference/toggle/obscurity_examine)) //ARK STATION EDIT
	var/face_obscured = (wear_mask && (wear_mask.flags_inv & HIDEFACE) && obscurity_examine_pref) || (head && (head.flags_inv & HIDEFACE) && obscurity_examine_pref) // ARK STATION EDIT
