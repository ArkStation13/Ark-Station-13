...
/obj/structure/mannequin
	...
	var/material
	/* ARK STATION EDIT REMOVAL - Extra Inventory
	...
	*/
	...
	var/static/list/slot_flags = list(
		...
		ITEM_SLOT_GLOVES,
		ITEM_SLOT_FEET,
		// ARK STATION EDIT ADDITION - Extra Inventory
		ITEM_SLOT_UNDERWEAR,
		ITEM_SLOT_SOCKS,
		ITEM_SLOT_SHIRT,
		ITEM_SLOT_BRA,
	...
	for(var/slot_flag in worn_items)
		...
		switch(text2num(slot_flag)) //this kinda sucks because build worn icon kinda sucks
			...
			if(ITEM_SLOT_WRISTS)
				...
				//default_icon = 'zov_modular_arkstation/_master_files/icons/mob/clothing/wrists.dmi' //No wrists icon yet
			// ARK STATION EDIT END
