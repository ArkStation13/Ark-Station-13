...
/proc/priority_announce(text, title = "", sound, type, sender_override, has_important_message = FALSE, list/mob/players = GLOB.player_list, encode_title = TRUE, encode_text = TRUE, color_override)
	...
	switch(type)
		...
		if(ANNOUNCEMENT_TYPE_CAPTAIN)
			header = MAJOR_ANNOUNCEMENT_TITLE("Ark Commander's Announcement") // ARK STATION EDIT
			GLOB.news_network.submit_article(text, "Ark Commander's Announcement", NEWSCASTER_STATION_ANNOUNCEMENTS, null) // ARK STATION EDIT
