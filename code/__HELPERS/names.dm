...
// ARK STATION EDIT START || RUSSIAN TRAITOR
					...
					if(1)
						if(names.len && prob(70))
							...
							if(prob(10))
								...
								. += new_name
					if(2)
						. += pick(GLOB.jobs) // ARK STATION EDIT || RUSSIAN TRAITOR
				safety -= 1
			if(2)
				switch(rand(1,2))//Food, drinks, or places. Only selectable once. // (БЫЛО 1,3)
					if(1)
						. += pick(GLOB.cocktails)
					if(2)
						. += pick(GLOB.locations)
