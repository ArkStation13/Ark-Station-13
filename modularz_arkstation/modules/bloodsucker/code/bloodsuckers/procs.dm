...
/datum/antagonist/bloodsucker/proc/break_masquerade(mob/admin)
	if(broke_masquerade)
		return
	owner.current.playsound_local(null, 'modularz_arkstation/modules/bloodsucker/sound/lunge_warn.ogg', 100, FALSE, pressure_affected = FALSE)
...
/datum/antagonist/bloodsucker/proc/return_vamp_examine(mob/living/viewer)
	...
	if(!isobserver(viewer) && (viewer.mind.has_antag_datum(/datum/antagonist/vassal) in vassals))
		...
		var/returnIcon = "[icon2html('modularz_arkstation/modules/bloodsucker/icons/misc/language.dmi', world, "bloodsucker")]"
		returnString += "\n"
