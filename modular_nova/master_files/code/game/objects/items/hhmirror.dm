	...
	switch(choice)
		...
		if("race")
			...
			if(HAS_TRAIT(human_user, TRAIT_MUTANT_COLORS) && !HAS_TRAIT(human_user, TRAIT_FIXED_MUTANT_COLORS))
				var/new_mutantcolor = tgui_color_picker(user, "Choose your skin color:", "Race change", human_user.dna.features["mcolor"]) // ARK STATION EDIT: TGUI COLOR PICKER
				if(new_mutantcolor)
		...
		if("hair")
			...
			if(hairchoice == "Style") // So you just want to use a mirror then?
				...
				var/new_hair_color = tgui_color_picker(human_user, "Choose your hair color", "Hair Color", human_user.hair_color) // ARK STATION EDIT: TGUI COLOR PICKER
				if(new_hair_color)
					human_user.hair_color = sanitize_hexcolor(new_hair_color)
					human_user.dna.update_ui_block(DNA_HAIR_COLOR_BLOCK)
				if(human_user.gender == "male")
					var/new_face_color = tgui_color_picker(human_user, "Choose your facial hair color", "Hair Color", human_user.facial_hair_color) // ARK STATION EDIT: TGUI COLOR PICKER
					if(new_face_color)
		...
		if(BODY_ZONE_PRECISE_EYES)
			var/new_eye_color = tgui_color_picker(human_user, "Choose your eye color", "Eye Color", human_user.eye_color_left) // ARK STATION EDIT: TGUI COLOR PICKER
			if(!user.can_perform_action(src, FORBID_TELEKINESIS_REACH))
				return TRUE
			if(new_eye_color)
