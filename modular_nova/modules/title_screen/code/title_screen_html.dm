...
/mob/dead/new_player/proc/get_title_html()
	...
	if(SSticker.current_state == GAME_STATE_STARTUP)
		...
		if(!SSticker || SSticker.current_state <= GAME_STATE_PREGAME)
			dat += {"<a id="ready" class="menu_button" href='?src=[text_ref(src)];toggle_ready=1'>[ready == PLAYER_READY_TO_PLAY ? "<span class='cta'>Ready</span>" : "Not Ready"]</a>"} // ARK STATION EDIT
		else
			dat += {"
