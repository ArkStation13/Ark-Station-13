...
/datum/admins/
	...
	var/MAKEANTAG_PL_DEFAULT_SECURITY = list(JOB_PRISONER, JOB_SECURITY_OFFICER, JOB_WARDEN, JOB_DETECTIVE, JOB_HEAD_OF_SECURITY, JOB_CAPTAIN, JOB_BLUESHIELD, JOB_BOUNCER)
	var/MAKEANTAG_PL_DEFAULT_HEADS = list(JOB_CAPTAIN,JOB_HEAD_OF_PERSONNEL,JOB_RESEARCH_DIRECTOR,JOB_CHIEF_ENGINEER,JOB_CHIEF_MEDICAL_OFFICER,JOB_HEAD_OF_SECURITY,JOB_QUARTERMASTER,JOB_CHAPLAIN) // ARK STATION EDIT
...
/datum/admins/proc/antag_get_protected_roles(antagtype)
	...
	switch(antagtype)
		...
		if(ROLE_CULTIST)
			...
			p_p += list(JOB_FISCAL, JOB_HEAD_OF_PERSONNEL) // ARK STATION EDIT
		if(ROLE_HERETIC)
			p_p += MAKEANTAG_PL_DEFAULT_SECURITY
			p_p += MAKEANTAG_PL_DEFAULT_HEADS
