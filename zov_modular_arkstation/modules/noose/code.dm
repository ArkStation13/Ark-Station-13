/obj/structure/chair/noose
	...
	icon_state = "noose"
	icon = 'zov_modular_arkstation/modules/noose/objects.dmi'
	layer = FLY_LAYER
	var/image/over
...
/obj/structure/chair/noose/user_buckle_mob(mob/living/carbon/human/M, mob/user, check_loc = TRUE)
	...
	if(do_after(user, user == M ? 10:100, M))
		if(buckle_mob(M))
			...
			playsound(user.loc, 'zov_modular_arkstation/modules/noose/noosed.ogg', 50, 1, -1)
	...
	for(var/m in buckled_mobs)
		...
		if(buckled_mob.has_gravity())
			...
			if(prob(20))
				...
				playsound(buckled_mob.loc, 'zov_modular_arkstation/modules/noose/noose_idle.ogg', 30, 1, -3)
