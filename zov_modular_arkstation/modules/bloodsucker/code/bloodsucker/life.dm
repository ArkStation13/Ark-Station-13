...
/datum/antagonist/bloodsucker/proc/HandleHealing(mult = 1)
	...
	if(owner.current.am_staked() || (HAS_TRAIT(owner.current, TRAIT_MASQUERADE) && !HAS_TRAIT(owner.current, TRAIT_NODEATH)))
		return FALSE
	owner.current.adjustBruteLoss(-1 * (actual_regen * 4) * mult, 0)
	owner.current.adjustFireLoss(-1 * (actual_regen * 4) * mult, 0)
	owner.current.adjustToxLoss(-1 * (actual_regen * 4) * mult, 0)
	owner.current.adjustOxyLoss(-1 * (actual_regen * 4) * mult, 0)
